// Pass a date object into this data element and it will reformat it to 'YYYYMMDD' (no hyphens).
// Arrays of date objects will be convered to a 'YYYYMMDD,YYYYMMDD,YYYYMMDD' string.
...
// Converts a single date object to YYYYMMDD:
function formatYYYYMMDD(singleDateObj) {
  ...
  if (typeof singleDateObj.getMonth === 'function') {
    ...
    mm = (mm < 10) ? '0' + mm : mm.toString(); // If no type coercion will occur, must manually convert at least one of these to a string or else it will perform a mathematical calculation during concatenation.
...
if (!!inputDate) {
  if (Array.isArray(inputDate)) {
    ...
    if (inputDate.length > 0 && typeof inputDate[0].getMonth === 'function') {
      // Converts an array of date objects to a string of YYYYMMDD,YYYYMMDD,YYYYMMDD:
